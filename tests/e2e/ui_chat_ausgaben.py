...
with sync_playwright() as p:
    ...
    check("Link „Zu meinen Ausgaben“ mit #ausgabe-<id>", zu.count() == 1 and zu.inner_text().strip() == "Zu meinen Ausgaben" and re.search(r"/ausgaben\?projekt=\d+#ausgabe-\d+", zu.get_attribute("href") or ""), zu.get_attribute("href") if zu.count() else "")
    ...
    check("Reiter-Zaehler um 1 erhoeht", pg.locator("#ausgabenTab").inner_text().strip() == f"Ausgaben ({vorher + 1})", pg.locator("#ausgabenTab").inner_text())
