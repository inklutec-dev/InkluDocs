...
with sync_playwright() as p:
    ...
    info = pg.locator("#projectHeadInfo").inner_text()
    check("Kopfzeile nennt Absaetze, Woerter und Uebersetzungsstand", "Absätze" in info and "Wörtern" in info and "Übersetzung Englisch" in info, info)
    ...
    check("Filter als fieldset mit legend und 3 Radios", fs.count() == 1 and fs.locator("legend").inner_text().strip() == "Absätze filtern" and fs.locator("input[type=radio]").count() == 3)
    ...
    check("Absatz-Ueberschriften h4 '{Art} N: Anfang' (>= 12 im Dokument)", alle_h4.count() >= 12 and (h4.first.inner_text().startswith("Titel ") or h4.first.inner_text().startswith("Überschrift ") or h4.first.inner_text().startswith("Absatz ")), (alle_h4.count(), h4.first.inner_text() if h4.count() else ""))
    check("Erster Absatz ist Nummer 1 (Haupttext vor Kopfzeile)", h4.first.inner_text().startswith("Titel 1:"), h4.first.inner_text())
    ...
    check("Original als Text mit Label", "Original:" in card.locator("p.seg-original").inner_text())
    ...
    check("Uebersetzung gefuellt und nicht gleich dem Original", ta.input_value().strip() and ta.input_value().strip() != card.locator("p.seg-original").inner_text().replace("Original:", "").strip(), ta.input_value()[:60])
    ...
    check("Dialog offen (natives dialog, aria-labelledby)", dlg.evaluate("d=>d.open") is True and dlg.get_attribute("aria-labelledby") == "uLaufHeading")
    check("Zielsprache: select mit Label, >= 10 Sprachen", pg.locator("label[for=uZielsprache]").count() == 1 and pg.locator("#uZielsprache option").count() >= 10)
    ...
    check("Rueckfrage nennt Absaetze, Woerter, Credits und Guthaben", "Absätze" in sm and "Wörtern" in sm and "Credits" in sm, sm)
    ...
    check("Abbrechen ist letzter Knopf im Dialog", ex.locator("button").last.inner_text().strip() == "Abbrechen")
