...
with sync_playwright() as p:
    ...
    check("Neues PDF-Projekt: Quickinfos in der Liste, aber ausgegraut (noch keine Felder)", len(pg.locator("#ansichtSelect option").all_text_contents()) == 4 and pg.locator("#ansichtSelect option[value=quickinfos]").get_attribute("disabled") is not None, pg.locator("#ansichtSelect option").all_text_contents())
    ...
    check("Ansichts-Wahl: alle vier Stationen waehlbar (mit Abschlussprüfung)", pg.locator("#ansichtSelect option").all_text_contents() == ["Dokument", "Alt-Texte", "Quickinfos", "Abschlussprüfung"] and pg.locator("#ansichtSelect option[disabled]").count() == 0, pg.locator("#ansichtSelect option").all_text_contents())
    axe(pg, "Dokument-Ansicht mit Formular")
    pg.select_option("#ansichtSelect", "quickinfos"); pg.click("#ansichtOeffnen")
    ...
    check("Quickinfo-Ansicht: Projektkopf mit PDF-Symbol + Ansichts-Wahl, ohne Statusanzeige", pg.locator(".projekt-kopf img.projekt-dateityp").count() == 1 and pg.locator(".projekt-kopf #ansichtSelect").count() == 1 and pg.locator("#projectStatusBadge").count() == 0)
