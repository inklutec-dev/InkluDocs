...
with sync_playwright() as p:
    ...
    check("Neues PDF-Projekt: Quickinfos in der Liste, aber ausgegraut (noch keine Felder)", len(pg.locator("#ansichtSelect option").all_text_contents()) == 3 and pg.locator("#ansichtSelect option[value=quickinfos]").get_attribute("disabled") is not None, pg.locator("#ansichtSelect option").all_text_contents())
    ...
    check("Karte nennt Formularfelder (12)", "Formularfelder" in dl and "12 Felder" in dl, dl)
    check("Knopf „Quickinfos bearbeiten“ auf der Karte", pg.locator("section.dok-karte button:has-text('Quickinfos bearbeiten')").count() == 1)
    check("Ansichts-Wahl: alle drei Stationen waehlbar", pg.locator("#ansichtSelect option").all_text_contents() == ["Dokument", "Alt-Texte", "Quickinfos"] and pg.locator("#ansichtSelect option[disabled]").count() == 0, pg.locator("#ansichtSelect option").all_text_contents())
    axe(pg, "Dokument-Ansicht mit Formular")
    pg.click("section.dok-karte button:has-text('Quickinfos bearbeiten')")
    ...
    check("Adresse ansicht=quickinfos, Ansichts-Wahl steht auf Quickinfos", "ansicht=quickinfos" in pg.url and pg.locator("#ansichtSelect").input_value() == "quickinfos")
    check("Upload-Hinweis in der Quickinfo-Ansicht spricht von Feldern", "Felder" in pg.locator("#projUploadHint").inner_text())
    check("Knopf „Quickinfos generieren“ vorhanden", pg.locator("#fGenAllBtn").count() == 1)
    axe(pg, "Quickinfo-Ansicht im PDF-Projekt")
    pg.goto(B + f"/app?projekt={pid}", wait_until="networkidle"); pg.wait_for_timeout(1000)
    check("Ohne ?ansicht: gemerkte Ansicht Quickinfos", pg.locator("#feldListe").count() == 1)
    pg.select_option("#ansichtSelect", "alttexte"); pg.click("#ansichtOeffnen"); pg.wait_for_selector("#bilderHeading", timeout=15000); pg.wait_for_timeout(500)
    # Das Testformular hat keine Bilder: die Alt-Text-Ansicht zeigt dann Upload-Block und Bilder-Anker, keine Feldliste.
    check("Wechsel zu Alt-Texte aus der Quickinfo-Ansicht", pg.locator("#bilderHeading").count() == 1 and pg.locator("#feldListe").count() == 0 and pg.locator("#ansichtSelect").input_value() == "alttexte")
