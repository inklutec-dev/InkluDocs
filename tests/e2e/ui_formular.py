...
with sync_playwright() as p:
    ...
    # Seit Michaels Mail „Neue Icons“ (14.09., 13:00): seine Formular-Grafik, in Schrifthoehe der Ueberschrift (20px), dekorativ.
    ico = pg.locator("#fExportPanel .export-doc-icon")
    check("Formular-Symbol rechts oben (Michaels Grafik), dekorativ, in Schrifthoehe der H2", ico.count() == 1 and (ico.get_attribute("src") or "").endswith("/icon-formular.png") and ico.get_attribute("alt") == "" and ico.get_attribute("aria-hidden") == "true" and abs(ico.evaluate("i => i.getBoundingClientRect().height") - pg.locator("#fExportHeading").evaluate("h => parseFloat(getComputedStyle(h).fontSize)")) < 1, (ico.get_attribute("src") if ico.count() else "fehlt", ico.evaluate("i => i.getBoundingClientRect().height") if ico.count() else 0))
