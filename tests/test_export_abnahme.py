...
from export_abnahme import abnahme_pdf, abnahme_loggen  # noqa: E402
...
class TestAbnahme(unittest.TestCase):
    def test_sauberer_export_besteht(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            a = abnahme_pdf(z, q, ["Blaues Quadrat auf weissem Grund"], erwartet_getaggt=r["tagged_count"])
    ...
    def test_gemeldeter_text_fehlt(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            a = abnahme_pdf(z, q, ["Blaues Quadrat", "Roter Kreis"], erwartet_getaggt=2)
    ...
    def test_waise_wird_erkannt(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            doc.close()
            a = abnahme_pdf(z, q, ["Blaues Quadrat"], erwartet_getaggt=1)
            self.assertFalse(a["ok"])
    ...
    def test_seitenzahl_abweichung(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            a = abnahme_pdf(os.path.join(d, "z2.pdf"), q, ["Blaues Quadrat"], erwartet_getaggt=1)
    ...
    def test_unbalancierte_marker(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            a = abnahme_pdf(os.path.join(d, "z3.pdf"), q, ["Blaues Quadrat"], erwartet_getaggt=1)
            self.assertFalse(a["ok"])
    ...
    def test_dekorativ_und_leer_zaehlen_nicht(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            xref = _quelle_mit_bild(q)
            _export(q, z, xref, "Blaues Quadrat")
            a = abnahme_pdf(z, q, ["Blaues Quadrat", "", "dekorativ", None], erwartet_getaggt=3)
            self.assertTrue(a["ok"], a)
