...
class TestEndpunkte(unittest.TestCase):
    ...
    def test_team_filter_und_mitglieder(self):
        ...
        d = self.c_voll.get("/api/admin/kunden", params={"filter": "free", "q": "team-mitglied"}).json()
        self.assertEqual([k["email"] for k in d["kunden"]], ["team-mitglied@example.invalid"])
    ...
    def test_kundenliste(self):
        ...
        d = self.c_voll.get("/api/admin/kunden", params={"filter": "single"}).json()
        self.assertTrue(all(k["plan"] == "single" for k in d["kunden"]))
