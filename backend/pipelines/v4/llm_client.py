...
Modellkennung eine Rolle bekommt, steuert die Umgebung:
  ...
  LLM_PROVIDER            Standard für alle Rollen: bedrock (Vorgabe), gemini oder openai
...
Die Modellkennungen je Anbieter kommen aus BEDROCK_MODEL_*, GEMINI_MODEL_* bzw. OPENAI_MODEL_*.
...
def _anbieter(rolle: str) -> str:
    ...
    return wert if wert in ('gemini', 'openai') else 'bedrock'
...
def _modell(rolle: str) -> str:
    ...
    if a == 'openai':
        return getattr(_openai, f'OPENAI_MODEL_{rolle}')
...
def ist_openai(model: str) -> bool:
    return (model or '').lower().startswith(('gpt-', 'o1', 'o3', 'o4'))
...
def call_with_schema(model, prompt, image_path, schema, max_tokens=1500, temperature=0.0, system=None):
    ...
    try:
        ...
        if ist_openai(model):
            return _openai.call_openai_with_schema(model=model, prompt=prompt, image_path=image_path, schema=schema,
                                                   max_tokens=max_tokens, temperature=temperature, system=system)
        return _bedrock.call_bedrock_with_schema(model=model, prompt=prompt, image_path=image_path, schema=schema,
                                                 max_tokens=max_tokens, temperature=temperature, system=system)
    except (_bedrock.BedrockCallError, _gemini.GeminiCallError, _openai.OpenAICallError) as e:
    ...
    try:
        if ist_gemini(model):
            return _gemini.call_gemini_text_with_schema(model=model, prompt=prompt, schema=schema,
                                                        max_tokens=max_tokens, temperature=temperature, system=system)
        if ist_openai(model):
            return _openai.call_openai_text_with_schema(model=model, prompt=prompt, schema=schema,
                                                        max_tokens=max_tokens, temperature=temperature, system=system)
