...
## 03.09.2026 — Prompt-Caching: Bilddaten ans Ende (ENV V4_PROMPT_CACHE)

Ausgangslage: Bedrock/Anthropic cachen nur einen byteidentischen ANFANG des Requests (tools -> system -> erster Content-Block). Im Combo-Prompt stand der erste bildabhaengige Wert (BILDGROESSE) nach ca. 14 % des Textes; ~90 % der Eingabe-Token (Regeln, Beispiele, Schema) wurden bei jedem Bild neu bezahlt.

Bauweise (bewusst als Schalter, Default aus = Prompts byteidentisch zu vorher):
- helpers.py: ContextVar-Schalter bilddaten_am_ende(); Renderer bildgroesse_zeile(), original_alt_zeile(), kontext_werte() liefern bei aktivem Schalter nur den festen Verweis "siehe Block BILDDATEN am Ende dieses Prompts"; bilddaten_block() rendert die echten Werte; BILDDATEN_MARKER trennt fest/variabel und erreicht das Modell nie.
- Builder (beschreibung_foto x6, beschreibung_daten x7, inventar, beschreibung_mini x3, classification): an den bildabhaengigen Stellen die Renderer statt Inline-Werte. Der WORTLAUT aller Regeln ist unveraendert; nur die Position der Bilddaten wandert. Combo ist unveraendert (erbt es von Inventar + Beschreibung).
- orchestrator.py, nur Lean-Pfad: with bilddaten_am_ende(_prompt_cache_an()) um die Builder-Aufrufe (Klassifikation, Mini, Combo), danach _mit_bilddaten() = Marker + Block; Nutzer-Prompt/Sprache/Variation folgen dahinter (variabel). Verify: Marker zwischen _basis und Namensregister.
- bedrock_client.py: _content_bloecke() -> [fester Text mit cache_control ephemeral] -> Bild -> [variabler Text]; ohne Marker wie bisher Bild -> Text. [BEDROCK-USAGE] loggt cache_read/cache_write. Converse-Weg (Nova): Marker wird entfernt, kein Caching.
- docker-compose(.staging).yml: V4_PROMPT_CACHE=\${V4_PROMPT_CACHE:-off}; Staging .env: on.
- Test: tests/test_prompt_cache.py (10): Schalter aus = Werte inline/kein Marker; an = fester Teil fuer zwei Bilder byteidentisch, Werte nur im Block; Client-Split.

Messung 03.09. (Probe zwei Personenfotos hintereinander): 1. Bild schreibt den Cache (Classify 4.831 + Combo 15.226 Token), 2. Bild liest ihn: nur 230 + 216 ungecachte Input-Token, Kosten 4,5 ct statt ~10 ct. Der Opus-Pruefpass cached NICHT (fester Teil ~1,6k Token unter dem Mindestpraefix des Modells) — hingenommen, der Gewinn liegt beim Erzeuger. Cache-TTL ~5 Minuten: greift bei Sammellaeufen und dichten Aufrufen, nicht beim ersten Bild nach laengerer Pause. Vollmessung: Quertest C1a/C1b (24 Bilder, kalt/warm) — siehe Bericht auf Steves Desktop.
