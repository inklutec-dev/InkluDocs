...
PREMIUM-BUILDER-DISZIPLIN (Steve+Claude+ChatGPT, 15.05.2026):
- Premium-Builder im Aufbau: diagramm (15.05.), tabelle (folgt),
  strukturformel (folgt).
- Konformitaet mit premium_prompt_standard.md (Memory + Desktop).
- Modus-aware: lean-Branch = Sonnet-optimiert (ChatGPT-iteriert).
  full-Branch = alter Mistral-Stand, bleibt als Fallback unangetastet.
- Helper-Architektur: AKTUELL INLINE, keine geteilten Helper.
  Refactor-Plan: nach tabelle + strukturformel die Sektionen vergleichen
  und nur ECHTE Wiederholung extrahieren (ChatGPT-Empfehlung 15.05.).
  Helper-Kandidaten sind im Code als '# === ... (Helper-Kandidat ...) ==='
  markiert.
- Schutz-Sektionen pro Builder unverhandelbar (LESBARE KONTAKTDATEN,
  INSIGHT-FIRST, KONSISTENZ, no-markdown-tables).
"""
...
from .helpers import load_examples, resolve_prompt_mode, user_hint_block
...
def build_beschreibung_prompt_diagramm(
    ...
    """Premium-Builder fuer diagramm — Sonnet-Lean-Iteration 15.05.2026.

    Vorher: Standard-Builder vom 14.04.2026 (v3.6 Mistral-Stil, im
    full-Branch unten erhalten).
    Neufassung durch ChatGPT + Steve + Claude:
    - Stil: beobachtend, Wissensvermittlung statt Datenpunkt-Aufzaehlung
    - DIAGRAMM-LOGIK-Sektion mit 6 Sub-Typen (Balken/Linie/Kreis/
      gestapelt/Streu/Heatmap)
    - Trend-Vokabular explizit
    - INSIGHT-FIRST mit GUT/SCHLECHT-Beispielen
    - Schutz-Sektionen erhalten (LESBARE KONTAKTDATEN, INSIGHT-FIRST,
      VOLLSTAENDIGKEITS-, KONSISTENZ-PFLICHT, no-markdown-tables)

    Konformitaet mit premium_prompt_standard.md (15.05.2026).
    Helper-Architektur: inline (Refactor-Plan: nach tabelle +
    strukturformel Sektionen vergleichen, dann extrahieren wenn
    echte Wiederholung sichtbar). Helper-Kandidaten sind im Code
    markiert.
    """
    schema_doc = render_schema_for_prompt(BeschreibungOutput)
...
"""
    ...
    # Full-Modus (Mistral): alter Standard-Stand vom 14.04.2026 bleibt
    # unangetastet als Fallback.
    return f"""{ROLE_BESCHREIBER}
